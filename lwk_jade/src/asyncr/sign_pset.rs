...
use crate::{
    create_jade_sign_req, derivation_path_to_vec, encode_value_for_jade,
    protocol::GetSignatureParams, script_code_wpkh, sign_liquid_tx::TxInputParams, Error,
};
...
impl<S: Stream<Error = Error>> Jade<S> {
    ...
    pub async fn sign(&self, pset: &mut PartiallySignedTransaction) -> Result<u32, Error> {
        ...
        for (i, input) in pset.inputs_mut().iter_mut().enumerate() {
            ...
            // Collect wallet keys for this input (may be empty for covenant/external inputs).
            let wallet_keys: Vec<_> = input
                .bip32_derivation
                .iter()
                .filter(|(_, (fingerprint, _))| fingerprint == &my_fingerprint)
                .map(|(pk, (_, path))| (*pk, derivation_path_to_vec(path)))
                .collect();

            if wallet_keys.is_empty() {
                // Non-wallet input (covenant / external UTXO).
                // Jade's state machine requires a tx_input for every input in the transaction;
                // sending path: [] tells it "not mine, skip signing".
                let value_commitment = encode_value_for_jade(&txout.value, i)?;
                let params = TxInputParams {
                    is_witness: true,
                    script_code: txout.script_pubkey.as_bytes().to_vec(),
                    value_commitment,
                    path: vec![],
                    sighash: None,
                    ae_host_commitment: vec![0u8; 32],
                };
                self.tx_input(params).await?;
            } else {
